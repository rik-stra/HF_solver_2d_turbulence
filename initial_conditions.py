...
from functions_for_solver import compute_VgradW_hat_np, get_psi_hat
import h5py

def get_initial_conditions(from_restart_file:bool, filters:Filters, grid:Grids, fname:str="none", USE_HDF5:bool=False, filter_restart_state=False, transpose = False):
    if from_restart_file:
        return initial_conditions_from_restart( fname, filters, grid, USE_HDF5, filter_restart_state, transpose)
    else:
        return create_initial_conditions(grid, filters)

def initial_conditions_from_restart( fname, filters:Filters, grid:Grids, USE_HDF5, filter_restart_state=False, transpose = False):
#####################   initial conditions ################################
    
    #fname = 'C:/Users/rik/Documents/Python Scripts/CAF_reduced_SGS-master/source/ref_data/restart_t_' + str(np.around(t_by_day,1))+file_extension
    ...
    return w_hat_n_HF, w_hat_nm1_HF, VgradW_hat_nm1_HF, psi_hat_n_HF, psi_hat_nm1_HF
...
def create_initial_conditions(grid:Grids, filters:Filters):
    ...
    #initial condition
    w_HF = np.sin(4.0*grid.x_HF)*np.sin(4.0*grid.y_HF) + 0.4*np.cos(3.0*grid.x_HF)*np.cos(3.0*grid.y_HF) + \
           0.3*np.cos(5.0*grid.x_HF)*np.cos(5.0*grid.y_HF) + 0.02*np.sin(grid.x_HF) + 0.02*np.cos(grid.y_HF)

    w_hat_n_HF = filters.P_HF*np.fft.fft2(w_HF)
